...
- Test suite using Node's built-in `node:test` runner (no extra dependencies). 60 tests run with `npm test`. Tests are anchored to documented specs rather than the implementation:
